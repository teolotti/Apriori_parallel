...
def support_worker_chunk(candidates_chunk, transactions_file):
    transactions = load_memmap(transactions_file)
...
def apriori_joblib_memmap(transactions_file, minsup, n_jobs=4, chunk_size=None):
    transactions = load_memmap(transactions_file)
    ...
    while candidates:
        ...
        support_count = count_support_joblib_chunks(candidates, transactions_file, n_jobs, chunk_size_eff)
...
def rules_single(support_data, min_conf, itemset_chunk):
    rules = []
        ...
        for ms in minsup_values:
            for n_jobs in n_jobs_list:
                for cs in chunk_size:
                    ...
                    frequent_itemsets, support_data = apriori_joblib_memmap(memmap_transaction_file, ms, n_jobs, cs)
                    ...
                    for _ in range(10):
                        ...
                        rules = generate_association_rules_joblib(frequent_itemsets, support_data_file, min_conf, n_jobs, cs)
        ...
        for ms in minsup_values:
            for n_jobs in n_jobs_list:
                for cs in chunk_size:
                    ...
                    for _ in range(10):
                        start_rules = time.perf_counter()
                        rules = generate_association_rules_joblib(frequent_itemsets, support_data_file, min_conf, n_jobs,
                                                                  cs)
                        end_rules = time.perf_counter()
                        rules_times.append(end_rules - start_rules)
